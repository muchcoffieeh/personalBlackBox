...
#define M2X_FEED_ID "<your M2X feed-id/device-id>"
#define M2X_API_KEY "<your M2X api key>"
...
int main()
{
    ...
    while (true) {
        ...
        if (!send_command(&mdot, "atsend config:", res))
            printf("failed to request config\r\n");

        if (!send_command(&mdot, "atsend subs:", res))
            printf("failed to request subscriptions\r\n");
